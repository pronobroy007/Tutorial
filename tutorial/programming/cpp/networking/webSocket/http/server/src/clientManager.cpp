...
void ClientManager::server_response(int index, char* path)
{
    ...
    header += "\r\n";
    send(_client_info_vec[index].socket, header.c_str(), header.size(), 0);
   ...
   std::cout << send(_client_info_vec[index].socket, "\r\n\r\n", 4, 0) << std::endl;
   input_file.close();
...
void ClientManager::send400(int index)
{
    const char *c400 = "HTTP/1.1 400 Bad Request\r\n"
        "Connection: close\r\n"
        "Content-Length: 11\r\n\r\nBad Request";
    send(_client_info_vec[index].socket, c400, strlen(c400), 0);
    drop_client(index);
}
...
void ClientManager::send404(int index)
{
    const char *c404 = "HTTP/1.1 404 Not Found\r\n"
        "Connection: close\r\n"
        "Content-Length: 9\r\n\r\nNot Found";
    send(_client_info_vec[index].socket, c404, strlen(c404), 0);
    drop_client(index);
}
